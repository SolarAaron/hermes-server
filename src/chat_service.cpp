...
namespace slr::hermes {
    ...
    chat_service::edit_message(grpc::ServerContext* context, const edit_message_request* request, operation_result* response) {
        ...
        user_query << "_id" << bsoncxx::oid(request->user_id().data(), request->user_id().size());
        ...
        auto pubkey_bytes = user_document_optional.value()["public_keys"].get_array().value[request->key_index()].get_binary();
        ...
        if(!verifier.VerifyMessage(
                                   (CryptoPP::byte*) request->nonce().data(),
                                   request->nonce().size(),
                                   (CryptoPP::byte*) request->signature().data(),
                                   request->signature().size()
                                  )) {
            return grpc::Status(grpc::StatusCode::UNAUTHENTICATED, "request_not_verified");
        }

        auto header_query = bsoncxx::builder::stream::document();

        header_query << "_id" << bsoncxx::oid(request->user_id().data(), request->user_id().size());
    ...
    chat_service::get_headers_after(grpc::ServerContext* context, const header_request* request, grpc::ServerWriter<operation_result>* writer) {
        ...
        header_query << "recipient" << bsoncxx::oid(request->user_id().data(), request->user_id().size());
